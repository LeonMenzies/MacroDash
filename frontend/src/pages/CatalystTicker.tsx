...
export default function CatalystTicker() {
  ...
  const [epics, setEpics] = useState<Record<EpicKey, EpicState>>({
    latent: { data: null, loading: false, error: null },
    definitive: { data: null, loading: false, error: null },
    horizon: { data: null, loading: false, error: null },
    macro: { data: null, loading: false, error: null },
  });
  ...
  const fetchEpic = useCallback(async (epic: EpicKey) => {
    ...
    if (cached && Date.now() - cached.ts < CACHE_TTL && cached.data[epic]) {
      setEpics((prev) => ({ ...prev, [epic]: { data: cached.data[epic], loading: false, error: null } }));
      return;
    }

    setEpics((prev) => ({ ...prev, [epic]: { ...prev[epic], loading: true, error: null } }));
    try {
      const res = await axios.get(`${API}/catalyst/${ticker}?epic=${epic}`);
      ...
      setEpics((prev) => ({ ...prev, [epic]: { data, loading: false, error: null } }));
    } catch (err: any) {
      setEpics((prev) => ({
        ...prev,
        [epic]: { data: null, loading: false, error: err.response?.data?.error || err.message },
      }));
    }
  }, [ticker]);
